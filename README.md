...
- 时间右侧：手机蓝牙连接、已设置闹钟、通知图标及数量（超过 9 显示 `+`）；有活动通知时点亮电话图标（未接来电是典型场景），无通知时可显示勿扰月亮图标。
...
身体电量用闪电图标，手表电量用电池图标。公开表盘接口没有未接来电计数，电话图标在存在活动通知时点亮（未接来电是典型场景），蓝牙图标单独表示手机已连接；通知数字是系统提供的活动通知数量。

## 低负荷策略

- 不启动 GPS，不订阅连续传感器，不创建后台服务、联网轮询或独立刷新定时器。
- 只有 `Positioning` 与 `SensorHistory` 权限。位置权限用于读取天气缓存中的观测位置，以计算日出/日落；不会主动申请 GPS 定位。
- 指标默认 60 秒读取一次，可选 120 / 300 秒；只读取当前显示槽位及小窗需要的传感器（血氧额外读取一条心率样本，用于判断是否剧烈运动）。重复指标只读一次，每次只取最新一条历史样本。
...
样本有效期：心率 5 分钟，身体电量/海拔/环境气压/压力 15 分钟，血氧 3 分钟；心率 ≥ 120 bpm（剧烈运动）时血氧按 1 分钟有效期判断。血氧并非连续测量，白天空白是正常情况。步数与热量为设备日累计值；日切时立即重新读取。
